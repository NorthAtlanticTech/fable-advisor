...
- **Grok lane (the default implementer):** the `grok-implementer` agent needs the [Cursor CLI](https://cursor.com/cli) installed and authenticated (install from [cursor.com/cli](https://cursor.com/cli), then `agent login`). It drives **Grok 4.5** headlessly (`agent -p … --model grok-4.5-xhigh --force`). Without it the agent reports `STATUS: unavailable` — it never silently falls back to a Claude model.
